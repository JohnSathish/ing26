<?php
/**
 * Migrate Don Bosco page from WordPress to local site
...
// Configuration
$WORDPRESS_URL = 'https://donboscoguwahati.org/index.php/don-bosco/';
$PAGE_SLUG = 'don-bosco';
$UPLOAD_DIR = __DIR__ . '/public_html/uploads/pages/';
$LOG_FILE = __DIR__ . '/don_bosco_migration_log.txt';
...
function downloadImage($url, $baseUrl, $prefix = 'don_bosco') {
    global $UPLOAD_DIR;
    ...
    try {
        ...
        if ($existing) {
            ...
            $insertStmt = $db->prepare("
                ...
                VALUES (:title, :slug, :content, :excerpt, :featured_image, 1, 1, '', 0, 0, NOW(), NOW())
            ");
            $insertStmt->execute([
                'title' => $data['title'],
                'slug' => $slug,
...
// Main execution
logMessage("Starting Don Bosco page migration from: $WORDPRESS_URL");
...
try {
    ...
    foreach ($images as $imageUrl) {
        ...
        $localPath = downloadImage($imageUrl, $WORDPRESS_URL, 'don_bosco');
        if ($localPath) {
