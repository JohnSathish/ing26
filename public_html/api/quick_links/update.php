...
$url = sanitizeInput($input['url'] ?? null, 'url');
...
try {
    ...
    $sql = "UPDATE quick_links SET " . implode(', ', $updates) . " WHERE id = :id";
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
    ...
    error_log("Quick link update error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Failed to update quick link']);
}
