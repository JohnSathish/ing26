...
/**
 * List Circulars
 * Public endpoint - returns active circulars with archive structure
 */
...
try {
    ...
    $sql = "
        SELECT id, title, month, year, file_path, description, is_active, created_at
        FROM circulars
        WHERE $whereClause
    ...
    // Get total count
    $countSql = "SELECT COUNT(*) as total FROM circulars WHERE $whereClause";
    ...
    // Get archive structure (years and months)
    ...
    error_log("Circulars list error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Failed to fetch circulars']);
}
