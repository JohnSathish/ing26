...
$location = sanitizeInput($input['location'] ?? null, 'string');
...
try {
    ...
    $sql = "UPDATE houses SET " . implode(', ', $updates) . " WHERE id = :id AND deleted_at IS NULL";
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
    ...
    error_log("House update error: " . $e->getMessage());
