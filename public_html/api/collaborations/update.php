...
$logo = sanitizeInput($input['logo'] ?? null, 'string');
...
try {
    ...
    $sql = "UPDATE collaborations SET " . implode(', ', $updates) . " WHERE id = :id";
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
    ...
    auditLog('update', 'collaboration', $id);
    ...
    error_log("Collaboration update error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Failed to update collaboration']);
}
