<?php
/**
 * Fetch Vice Provincial page content and download all images
 * This script fetches from the provincials table and downloads images
 */
...
$LOG_FILE = __DIR__ . '/vice_provincial_images_log.txt';
...
try {
    ...
    if (!$viceProvincial) {
        logMessage("Vice Provincial not found in database");
        exit(1);
    }
    ...
    // Also check if there's a dynamic page with slug 'vice-provincial'
    $pageStmt = $db->prepare("
    ...
    if ($page) {
        ...
        if ($imagesUpdated > 0 || $updatedPageContent !== $page['content'] || $updatedPageFeaturedImage !== $page['featured_image']) {
            ...
            logMessage("✓ Updated dynamic page with downloaded images");
        }
    }
